...
gst_va_filter_compose (GstVaFilter * self, GstVaComposeTransaction * tx)
{
  ...
  for (; sample; sample = tx->next (tx->user_data)) {
    ...
    /* cross-display DMABuf wrappers carry GstParentBufferMeta; their
     * VASurfaceID is tied to the wrapper's lifetime so hold until vaEndPicture */
    if (gst_buffer_get_parent_buffer_meta (sample->buffer))
